...
public class GameController {
    ...
    public String allUsersPost(Model model, @RequestParam Long id) {
        quizService.addQuiz(QuizDto.builder().user(userService.findOneUser(id).get()).date(LocalDateTime.now()).build());
        ...
        model.addAttribute("levelList", levelService.findAllLevel());
    ...
    public String settingGamePost(Model model, @ModelAttribute QuestionFilterDto questionFilter) {
        List<QuestionDto> questionList = questionService.random5question(questionService.findAllQuestionAfterFiltrs(questionFilter));
